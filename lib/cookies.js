...
module.exports.parseSetCookies = (url, setCookieHeader, logging = false) => {
    if(logging) console.log("Parsing:", setCookieHeader);
...
module.exports.filterCookies = () => {
    for (const url of allCookies.keys()) {
        ...
        for (const cookie of allCookies.get(url)) {
            ...
            if (keysList[cookie.name] && keysList[cookie.name].parsedAt > cookie.parsedAt)
                continue;
